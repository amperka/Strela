...
namespace {
    ...
    void _setMotorDirections(bool direction_1, bool direction_2) {
    ...
    void _setMotorDirection(uint8_t mot, bool direction) {
        if (mot == M1) {
            if (direction ^ _motorConnection_1)
                PORTD |= _BV(4);  //digitalWrite(4, HIGH)
            else
...
void motorConnection( bool direction_1, bool direction_2) {
    STRELA_INIT_CHECK;
...
void drive( int motorSpeed_1, int motorSpeed_2) {
    STRELA_INIT_CHECK;
